...
map_fema_data <- function(df, x_var = "state", by = "incidentType") {
  ...
  if(.n_by > 12) {
    morecolors <- c(RColorBrewer::brewer.pal(12, "Paired"),
  ...
  MapGG <- ggplot2::ggplot(data = df, ggplot2::aes_string(x = x_var)) + 
    ggplot2::geom_bar(ggplot2::aes_string(fill = by), position = 
                        ggplot2::position_stack(), show.legend=TRUE) + 
    ggplot2::scale_fill_manual(values = morecolors) + ggplot2::theme_minimal() +
